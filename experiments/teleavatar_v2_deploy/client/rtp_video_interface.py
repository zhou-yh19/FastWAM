...
# Do NOT call Gst.init at import time: importing this module after/before rclpy
# with an eager Gst.init is a common source of hard segfaults on some hosts.
_GST_READY = False
...
class RTPH265VideoInterface:
    ...
    def _check_udp_port_available(self) -> None:
        """Fail early when another process already owns the RTP port.

        GStreamer may allow multiple UDP sockets to bind the same port. In that
        case the kernel can deliver packets to only one receiver, which otherwise
        looks like a missing robot stream to this process.
        """
        ...
        try:
            ...
            raise RuntimeError(
                f"RTP UDP port {self.port} is already in use. Stop the other "
                "RTP receiver, then check with: "
                f"ss -lunp | grep {self.port}"
            ) from exc
        finally:
            probe.close()
    ...
    def wait_for_initial_data(self, timeout: float = 10.0) -> bool:
        ...
        while not self._first_frame.is_set():
            if self._eos_or_error.wait(timeout=min(0.1, max(0.0, deadline - time.monotonic()))):
                self.logger.error(
                    "RTP pipeline ended before the first frame: "
                    "rtp_packets=%d h265_buffers=%d",
    ...
    def get_views_if_newer(
        self,
        views: Iterable[str],
        since: Optional[Dict[str, float]] = None,
    ) -> Dict[str, tuple[np.ndarray, float]]:
        """Return {view: (rgb_copy, stamp)} only for views with a frame newer than `since`.

        Lets a poller pick up each decoded frame exactly once without copying the
        views it does not need (notably the full SBS composite).
        """
        since = since or {}
        fresh: Dict[str, tuple[np.ndarray, float]] = {}
        with self.lock:
            for view in views:
                stamp = self.image_timestamps.get(view)
                image = self.latest_images.get(view)
                if stamp is None or image is None:
                    continue
                previous = since.get(view)
                if previous is not None and stamp <= previous:
                    continue
                fresh[view] = (image.copy(), float(stamp))
        return fresh
